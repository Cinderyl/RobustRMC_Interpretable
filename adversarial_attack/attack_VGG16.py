...
model.add(Conv2D(64, (3, 2), padding='same', kernel_regularizer=regularizers.l2(weight_decay)))
...
model.add(Conv2D(128, (3, 2), padding='same', kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation('relu'))
model.add(BatchNormalization())
...
model.add(Conv2D(256, (3, 2), padding='same', kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation('relu'))
model.add(BatchNormalization())
model.add(Dropout(0.4))
model.add(Conv2D(256, (3, 2), padding='same', kernel_regularizer=regularizers.l2(weight_decay)))
...
model.add(BatchNormalization())
model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
model.add(Conv2D(512, (3, 2), padding='same', kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation('relu'))
model.add(BatchNormalization())
model.add(Dropout(0.4))
model.add(Conv2D(512, (3, 2), padding='same', kernel_regularizer=regularizers.l2(weight_decay)))
...
filepath = '/data0/quziwen/yelinhui/explainable/Deepsig.10A/vgg16/mytest/128_2/对抗训练vgg16_128_21.h5'
...
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_VGG16/target_attack/label_{}_to_{}/".format(c, target_label[c])
...
#     count_index = np.array(count_index)
#     source_signal = right_train[count_index]
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_VGG16/un_target_attack/label_{}/".format(c)
#     os.makedirs(path, exist_ok=True)
#
...
'''
# dirs_path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_VGG16/target_attack/"
...
#
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_VGG16/target_v/"
#     os.makedirs(path, exist_ok=True)
#
...
#         cam, cam_wight = grad_cam(model, img, int(source_label), 'conv2d_10')
...
#         需要注意的是相位和幅值的权重是一样的
#         '''
#         cam, cam_wight = grad_cam(model, img, int(ad_label), 'conv2d_10')
#         radio11CamweightTestX = cam_wight
#
#         '''
#         权重归一化
...
'''
循环画无目标攻击的原样本波形图和对抗样本波形图（或者画对抗训练后的图）
'''
dirs_path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_VGG16/un_target_attack/"
dirs = os.listdir(dirs_path)
for dir in dirs:
    dir_path = dirs_path+dir
    files = glob.glob(join(dir_path, f'*.npy'))
    ...
    path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_VGG16/对抗训练的可视化图/"
    os.makedirs(path, exist_ok=True)
...
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_VGG16/无目标攻击--用于对抗训练/label_{}/".format(c)
#     os.makedirs(path, exist_ok=True)
#
#     ad_to_save_path = path + "label_{}_ad_siganl_save.npy".format(c)
#     source_to_save_path = path + "label_{}_source_signal.npy".format(c)
