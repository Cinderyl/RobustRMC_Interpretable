...
def build_model():
    ...
    model.add(Conv2D(192, (5, 2), padding='same', kernel_regularizer=keras.regularizers.l2(weight_decay)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))

    model.add(Conv2D(160, (1, 2), padding='same', kernel_regularizer=keras.regularizers.l2(weight_decay)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))

    model.add(Conv2D(96, (1, 2), padding='same', kernel_regularizer=keras.regularizers.l2(weight_decay)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))

    model.add(MaxPooling2D(pool_size=(3, 2), strides=2, padding='same'))
    ...
    model.add(Conv2D(192, (1, 2), padding='same', kernel_regularizer=keras.regularizers.l2(weight_decay)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    ...
    model.add(Conv2D(192, (3, 2), padding='same', kernel_regularizer=keras.regularizers.l2(weight_decay)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    ...
    model.add(Conv2D(11, (1, 2), padding='same', kernel_regularizer=keras.regularizers.l2(weight_decay)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
...
filepath = '/data0/quziwen/yelinhui/explainable/Deepsig.10A/NIN/mytest/model_128_2/对抗训练_NIN_128_22.h5'  # 所要保存的文件名字，h5格式，不用写路径，默认在程序执行的文件夹内
...
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_NIN/target_attack/label_{}_to_{}/".format(c, target_label[c])
...
#     count_index = np.array(count_index)
#     source_signal = right_train[count_index]
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_NIN/un_target_attack/label_{}/".format(c)
#     os.makedirs(path, exist_ok=True)
#
...
'''
# dirs_path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_NIN/target_attack/"
...
#
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_NIN/target_v/"
#     os.makedirs(path, exist_ok=True)
#
...
#         cam, cam_wight = grad_cam(model, img, int(source_label), 'conv2d_9')
...
#         需要注意的是相位和幅值的权重是一样的
#         '''
#         cam, cam_wight = grad_cam(model, img, int(ad_label), 'conv2d_9')
#         radio11CamweightTestX = cam_wight
#
#         '''
#         权重归一化
...
'''
循环画无目标攻击的原样本波形图和对抗样本波形图（或画对抗训练的可视化图）
'''
import glob
dirs_path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_NIN/un_target_attack/"
dirs = os.listdir(dirs_path)
for dir in dirs:
    dir_path = dirs_path+dir
    files = glob.glob(join(dir_path, f'*.npy'))
    ...
    path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_NIN/对抗训练后的可视化图/"
    os.makedirs(path, exist_ok=True)
...
#     path = "/data0/quziwen/yelinhui/explainable/Deepsig.10A/adversarial_attack/attack_NIN/无目标攻击--用于对抗训练/label_{}/".format(c)
#     os.makedirs(path, exist_ok=True)
#
#     ad_to_save_path = path + "label_{}_ad_siganl_save.npy".format(c)
#     source_to_save_path = path + "label_{}_source_signal.npy".format(c)
