...
net_lstm1 = tflearn.lstm(input_layer, n_units=128, dropout=0.8, dynamic=True, return_seq=True)

net_lstm1 = tf.transpose(net_lstm1, [1,0,2])

print(net_lstm1[0].get_shape())

net_lstm2 = tflearn.lstm(net_lstm1, n_units=128, dropout=0.8, dynamic=True, return_seq=True)
net_lstm2 = tf.transpose(net_lstm2, [1,0,2])
print(net_lstm2[0].get_shape())

net_output = tflearn.fully_connected(net_lstm2, 12, activation='softmax')
net = tflearn.regression(net_output, optimizer='adam', learning_rate=0.0001,
                         loss='categorical_crossentropy')

# Training

model = tflearn.DNN(net, tensorboard_verbose=0, tensorboard_dir="./logs", max_checkpoints=3, best_checkpoint_path="2lstm_128_64units")
#model.load('model.tfl')

model.fit(trainX, trainY, validation_set=(valX, valY), show_metric=True,batch_size=batch_size, n_epoch=50, shuffle=True,run_id='2lstm_128_64units')
end = time.time()
print (end-start)
