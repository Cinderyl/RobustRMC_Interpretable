...
net_lstm1 = tflearn.lstm(input_layer, n_units=32, dropout=0.8, dynamic=True, return_seq=True)

net_lstm1 = tf.transpose(net_lstm1, [1,0,2])

print(net_lstm1[0].get_shape())

net_output = tflearn.fully_connected(net_lstm1, num_classes, activation='softmax')
net = tflearn.regression(net_output, optimizer='adam', learning_rate=0.0001,
                         loss='categorical_crossentropy')

# Training

model = tflearn.DNN(net, tensorboard_verbose=0, tensorboard_dir="./logs", max_checkpoints=3, best_checkpoint_path="lstm_32units")
#model.load('model.tfl')

model.fit(trainX, trainY, validation_set=(valX, valY), show_metric=True,batch_size=batch_size, n_epoch=50, shuffle=True,run_id='lstm_32units')
